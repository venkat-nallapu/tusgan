...
# ---------------------------------------------------------------------------
# Path setup — allow importing Generator from v3/
# ---------------------------------------------------------------------------
_WGAN_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "v3")
...
@st.cache_resource
def load_model_and_data(ckpt_path: str = CHECKPOINT_PATH, data_path: str = DATA_NPZ_PATH):
...
def page_generate(G, cfg, data):
    ...
    # --- Info blurb ---
    st.info(
        "**How it works (TUS-GAN v3):** A demographic conditioning vector is combined with your custom "
...
def main():
    ...
    version = st.sidebar.selectbox(
        "Model Version", ["TUS-GAN v3 (Baseline)", "TUS-GAN v4 (Upgraded)"], index=1
    )
    ...
    G, cfg, data = load_model_and_data(ckpt_path, data_path)
