...
def load_generator(checkpoint_path: str, data_path: str, device: torch.device):
    ...
    print(
        f"   cond_dim={actual_cond_dim}, districts={actual_num_districts}, states={actual_num_states}"
    )
    return G, cfg
...
def compute_numpy_transition_matrix(codes: np.ndarray) -> np.ndarray:
    """
    Compute the transition probability matrix (9x9) for activity codes (1..9).
    Row i represents the source activity, column j represents the destination.
    """
    # codes shape: (N, 48)
    c_t = codes[:, :-1] - 1  # shape (N, 47), values 0..8
    c_tp1 = codes[:, 1:] - 1  # shape (N, 47), values 0..8

    trans = np.zeros((9, 9), dtype=float)
    # Vectorised index accumulation using np.add.at
    np.add.at(trans, (c_t, c_tp1), 1.0)

    # Normalize rows to form a transition probability matrix
...
def compute_metrics(real_codes: np.ndarray, fake_codes: np.ndarray):
    ...
    Also computes the transition matrix Frobenius difference.
    ...
    # ── Transition Matrix Comparison (v3) ─────────────────────
    trans_real = compute_numpy_transition_matrix(real_codes)
    trans_fake = compute_numpy_transition_matrix(fake_codes)
    trans_diff = trans_fake - trans_real
    trans_fnorm = float(np.linalg.norm(trans_diff, ord="fro"))
...
def plot_frequency_distribution(m: dict, output_dir: str):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"{d}\n{DIVISION_LABELS[d][:12]}" for d in m["divisions"]], fontsize=8)
    ax.legend()
