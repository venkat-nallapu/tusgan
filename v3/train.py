...
def load_checkpoint(path, G, D, opt_G, opt_D, device):
    ...
    # Load regular G weights. If v3 checkpoint is loaded later, we can also load the G_state.
...
def train(cfg):
    ...
    # Precompute real transition matrix from the entire dataset
    print("Computing real transition matrix from dataset...")
    with torch.no_grad():
        real_probs_all = (dataset.diary.squeeze(-1) + 1.0) / 2.0
        x_t = real_probs_all[..., :-1]
        x_tp1 = real_probs_all[..., 1:]
        trans_real_all = torch.einsum("b i t, b j t -> i j", x_t, x_tp1)
        row_sums = trans_real_all.sum(dim=1, keepdim=True)
        P_real = (trans_real_all / (row_sums + 1e-8)).to(device)
    print("Real transition matrix computed.")
    ...
    for epoch in range(start_epoch, cfg["epochs"] + 1):
        ...
        for gen_step in range(n_gen_batches):
            ...
            # Compute transition matrix consistency loss (v3)
            probs_fake = (fake_diaries.squeeze(-1) + 1.0) / 2.0  # shape (B, 9, 48)
            x_t_f = probs_fake[..., :-1]
            x_tp1_f = probs_fake[..., 1:]
            trans_fake = torch.einsum("b i t, b j t -> i j", x_t_f, x_tp1_f)
            row_sums_f = trans_fake.sum(dim=1, keepdim=True)
            P_fake = trans_fake / (row_sums_f + 1e-8)
            loss_trans = torch.mean((P_fake - P_real) ** 2)

            # Total Generator Loss = WGAN Adversarial Loss + Transition Consistency Loss
            loss_G = generator_loss(fake_scores) + cfg["lambda_transition"] * loss_trans
