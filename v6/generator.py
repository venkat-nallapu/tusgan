...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, max_len: int = 48):
        super().__init__()
...
class TemporalTransformerBlock(nn.Module):
    def __init__(self, channels: int, num_layers: int = 6, nhead: int = 8):
        super().__init__()
        self.channels = channels
        self.pos_embed = PositionalEncoding(channels, max_len=48)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply transformer positional encoding
...
class Generator(nn.Module):
    def __init__(
        ...
        self.transformer1 = TemporalTransformerBlock(base_channels // 2, num_layers=6, nhead=8)
