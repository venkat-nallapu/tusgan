...
def get_config():
    return dict(
        ...
        infonce_temp=0.1,
        lambda_duration=1.0,
        # v6 neuro-symbolic logic constraint
        lambda_logic=10.0,
    )
...
def compute_neuro_symbolic_loss(probs_fake, cond_vector):
    """
    Neuro-symbolic soft penalty for child labor (v4 implementation).
    Provides a continuous gradient away from forbidden states.
    """
    is_child = cond_vector[:, 0] == 1
    if not is_child.any():
        return torch.tensor(0.0, device=probs_fake.device)
    
    # Penalize the soft probability of employment (index 0) for children
    child_emp_probs = probs_fake[is_child, 0, :]
    loss = child_emp_probs.sum() / (is_child.sum() + 1e-8)
    return loss
...
def train(cfg):
    ...
    for epoch in range(start_epoch, cfg["epochs"] + 1):
        ...
        for gen_step in range(n_gen_batches):
            ...
            # 4. Neuro-Symbolic Logic Loss (Soft Penalty)
        ...
        avg_loss_logic = epoch_loss_logic / n_gen_steps if n_gen_steps > 0 else 0
        ...
        writer.add_scalar("Epoch/loss_logic", avg_loss_logic, epoch)
        ...
        print(
            ...
            f"L_Trans: {avg_loss_trans:.5f} | L_Dur: {avg_loss_duration:.5f} | "
            f"L_Info: {avg_loss_infonce:.5f} | L_Log: {avg_loss_logic:.5f} | "
